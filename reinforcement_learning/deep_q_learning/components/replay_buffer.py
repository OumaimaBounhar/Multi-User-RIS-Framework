...
class ReplayBuffer():
    ...
    def __init__(self, replay_buffer_memory_size: int, batch_size: int, input_dims, device = 'cpu'):
        ...
        self.terminated_memory  = np.zeros([replay_buffer_memory_size,1], dtype=np.float32)
        
        #Index to keep track of where to store the data
        self.idx = 0
        
    def store_transition(self, current_state, action, reward, next_state, terminated):
        ...
        self.terminated_memory [self.idx] = float(terminated) # (float(True) -> 1.0 -> true terminal state / float(False) -> 0.0 -> truncated)
    ...
    def sample_buffer(self):
        ...
        return current_state_batch, action_batch, reward_batch, next_state_batch, terminated_batch